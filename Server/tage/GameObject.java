...
public class GameObject
{
	...
	*/
	...
	/** spawns object on the 'map' given the parent, shape, texture, position, scale, and rotation
	 * @param type      the type GameObject to attach the new object
	 * @param shape     the ObjShape used to define the objects shape
	 * @param texture   the TextureImage applied onto the object
	 * @param tx        local translation along X-axis
	 * @param ty        local translation along Y-axis
	 * @param tz        local translation along Z-axis
	 * @param scale     the scale applied to the object
	 * @param rotation  rotation around the y-axis
	 *
	 * @return the newly created GameObject
	 */
	public static GameObject spawnObject(GameObject type, ObjShape shape, TextureImage texture, float tx, float ty, float tz, float scale, float rotation) {
		Matrix4f initialTranslation, initialScale, initialRotation;

		GameObject obj = new GameObject(type, shape, texture);
		initialTranslation = (new Matrix4f()).translation(tx, ty, tz);
		initialScale = (new Matrix4f()).scaling(scale);
		initialRotation = (new Matrix4f()).rotationY((float)java.lang.Math.toRadians(rotation));
	...
	/** rotates the GameObject around its local Y-axis */
	public void globalYaw(float angle) {
		Matrix4f rot = new Matrix4f().rotateY(angle);
		rot.mul(localRotation, localRotation);
		update();
	}

	/** rotates the GameObject around its local X-axis */
	public void pitch(float angle) {
		Matrix4f rot = new Matrix4f().rotation(angle, new Vector3f(1, 0, 0));
		rot.mul(localRotation, localRotation);
		update();
	}
}